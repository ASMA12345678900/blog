...
@section('content')
    ...
    <div class="container  mx-auto  pt-15 pb-5">
        <form  action="/blog/{{$post->slug }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="text" name="title" value="{{ $post->title }}" class="w-full h-20 text-6xl rounded-lg shadow-lg border-b p-15 mb-5">
            <textarea name="description"   class="w-full h-60 text-6l rounded-lg shadow-lg border-b p-15 mb-5" >{{ $post->description }}</textarea>
